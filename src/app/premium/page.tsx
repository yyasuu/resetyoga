...
export const metadata = { title: 'Reset Yoga Premium | 審査制・マッチング型プレミアムヨガ' }

const CATEGORIES = [
  { id: 'desk',      icon: '💻', name_ja: 'デスクワーカー回復',    name_en: 'Desk Recovery',       desc_ja: '肩・首・腰の不調をピンポイントで解決' },
  { id: 'sleep',     icon: '🌙', name_ja: '睡眠・神経系',          name_en: 'Sleep & Nervous System', desc_ja: '自律神経を整え、深い眠りへ' },
  { id: 'womens',    icon: '🌸', name_ja: '女性のバランス',         name_en: "Women's Balance",     desc_ja: '産前産後・ホルモンバランス・女性特有の不調' },
  { id: 'elite',     icon: '🏆', name_ja: 'エリートマスター',       name_en: 'Elite Yogi / Master', desc_ja: '深い専門性と実績を持つ上級講師' },
  { id: 'beginner',  icon: '🌱', name_ja: 'ビギナーガイド',         name_en: 'Gentle Beginner',     desc_ja: '初めてでも安心、丁寧な指導' },
  { id: 'athletic',  icon: '⚡', name_ja: 'アスリートモビリティ',   name_en: 'Athletic Flow',       desc_ja: '筋力・可動域・パフォーマンス向上' },
  { id: 'luxury',    icon: '💎', name_ja: 'マインドフルラグジュアリー', name_en: 'Mindful Luxury',   desc_ja: 'リトリートのような上質な体験' },
  { id: 'english',   icon: '🌍', name_ja: 'English Global Class',  name_en: 'English Global',      desc_ja: '英語で受講できる国際クラス' },
  { id: 'celebrity', icon: '⭐', name_ja: 'セレブリティ講師',       name_en: 'Celebrity / Influencer', desc_ja: 'フォロワー数万人の人気講師' },
  { id: 'japanese',  icon: '🎋', name_ja: 'ジャパニーズカーム',     name_en: 'Japanese Calm',       desc_ja: '静けさを大切にした、日本語の丁寧なクラス' },
]

const TIERS = [
  { id: 'tier1', badge: 'Tier 1', name: '一般Premium講師',    price: '¥2,500〜4,500',  desc: '認定資格を持ち、品質審査を通過した講師' },
  { id: 'tier2', badge: 'Tier 2', name: '認定・高評価講師',   price: '¥4,500〜8,000',  desc: '継続率・満足度が高く、実績のある専門講師' },
  { id: 'tier3', badge: 'Tier 3', name: '有名講師・希少テーマ', price: '¥8,000〜20,000', desc: '希少な専門性を持つ、または著名な認定講師' },
  { id: 'tier4', badge: 'Tier 4', name: '1on1・少人数特別枠', price: '¥15,000〜50,000', desc: 'マンツーマン・プライベートの完全個別指導' },
]
...
const PLANS = [
  {
    ...
    period: '/月',
    highlight: false,
    features: ['月8回までPremiumクラス', 'Tier 1〜2講師を中心に', '優先予約', 'アーカイブ一部開放'],
  },
  {
    id: 'studio_pass',
    ...
    period: '/月',
    highlight: true,
    features: ['月10〜15クレジット', 'Tier 1〜3全講師アクセス', 'マッチングサポート', '限定シリーズ参加権', '優先予約'],
  },
  {
    id: 'signature',
    ...
    period: '/4週間',
    highlight: false,
    features: ['テーマ集中型プログラム', '小グループ（定員6名）', '課題＋チャットフォロー', '修了証書'],
  },
  {
    id: 'private',
    ...
    period: '/回',
    highlight: false,
    features: ['講師Tierで価格変動', '完全マンツーマン', '自分に合わせたプログラム', 'チャットでの事前相談'],
  },
]
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {TIERS.map((tier, i) => (
              <div key={tier.id} className={`rounded-2xl p-5 border ${i === 1 ? 'border-sage-400 bg-sage-50 dark:bg-sage-900/20 dark:border-sage-600' : 'border-gray-100 dark:border-navy-700 bg-linen-50 dark:bg-navy-700'}`}>
                ...
                <p className="font-bold text-navy-800 dark:text-white mt-1 mb-1 text-sm">{tier.name}</p>
                <p className="text-lg font-bold text-sage-600 dark:text-sage-400 mb-2">{tier.price}<span className="text-xs font-normal text-gray-400">/クラス</span></p>
                <p className="text-xs text-gray-500 dark:text-navy-300 leading-relaxed">{tier.desc}</p>
              </div>
            ))}
          </div>
